...
### 2026-09-08（資格一覧の検索タイトル・摘要）

- 数据来源与截止：本轮输入的 GSC，2026-08-08～2026-09-04；未重新拉取。全站 491 展示、10 点击；`/exams/` 20 展示、0 点击、平均排名 7.8，样本较小。
- 单一动作 / CTR 杠杆：仅修改 `/exams/` 的 title 与 description，突出衛生管理者過去問与乙4免费问题，并明确乙4为原创题，不暗示提供乙4官方真题。已有面包屑、正文、内链、路由和索引规则不变；不导入题库、不修改考试事实或答案。
- 验证：`npm run build` 通过（24 文件，0 errors / warnings / hints）；定向断言确认正文与内链不变、唯一 H1、现有面包屑及 SSR 构建产物包含新元数据。初次产物断言仅扫描 `.js` 而遗漏 `.mjs`，修正检查范围后通过。`git diff --check` 通过。未运行浏览器响应式/控制台检查（本轮仅 head 元数据，无可见布局改动）；未进行线上验证。
- 部署版本：未 commit / push / deploy，由外层 runner 处理；保留预存的未跟踪 `node_modules` 链接。
- 观察窗口：上线后 7 天初查，21 天比较 `/exams/` GSC 展示、点击、CTR 和平均排名；CTR 1%+ 为实验目标而非保证。后续结果待收集，低样本下不作因果归因。
